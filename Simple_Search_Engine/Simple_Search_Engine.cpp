...
std::vector<int> findTopThreeSumIDF(const std::string& line, const std::vector<std::pair<int, std::string>>& corpus, const std::unordered_map<std::string, double>& idfMap) {
	...
	for (int i = 0; i < std::min(3, static_cast<int>(sumIDFList.size())); ++i) {
		topThreeIDs.push_back(sumIDFList[i].first);
	}

	// If there are less than three document IDs, fill the remaining slots with -1
	while (topThreeIDs.size() < 3) {
		topThreeIDs.push_back(-1);
	}
...
int main(int argc, char* argv[]) {
	...
	std::ifstream corpusFile("C:\\Users\\chius\\OneDrive\\桌面\\Simple_Search_Engine_2\\corpus1.txt");
	if (!corpusFile) {
	...
	std::ifstream queriesFile("C:\\Users\\chius\\OneDrive\\桌面\\Simple_Search_Engine_2\\query1.txt"); 
	if (!queriesFile) {
	...
	while (std::getline(queriesFile, line)) {
		std::vector<int> topThreeIDs = findTopThreeSumIDF(line, corpus, idfMap);

		for (int i = 0; i < top_k; ++i) {
			if (i == 0) {
				std::cout << topThreeIDs[i];
			}
			else {
				std::cout << " " << topThreeIDs[i];
			}
